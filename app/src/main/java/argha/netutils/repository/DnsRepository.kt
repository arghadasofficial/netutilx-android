...
import argha.netutils.models.dnsx.DnsData
...
class DnsRepository @Inject constructor(private val dnsAPI: DnsAPI) {
    ...
    private val _dnsTypes = MutableStateFlow(DnsTypes(emptyList(), "", false))
    val dnsTypes: StateFlow<DnsTypes> = _dnsTypes

    private val _dnsQueryServers = MutableStateFlow(DnsServers(emptyList(), "", false))
    val dnsQueryServers: StateFlow<DnsServers> = _dnsQueryServers

    private val _dnsInfo = MutableStateFlow(DnsInfo(emptyList(), "", false))
    val dnsInfo: StateFlow<DnsInfo> = _dnsInfo

    private val _soaInfo = MutableStateFlow(SOAInfo(emptyList(), "", false))
    val soaInfo : StateFlow<SOAInfo> = _soaInfo

    private val _allDnsInfo = MutableStateFlow(
        DnsAllInfo(
            DnsData(
                emptyList(),
                emptyList(),
